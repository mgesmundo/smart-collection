  ...
  if (items) {
    if (_.isArray(items)) {
      ...
      function pause(cancel) {
        if (cancel) {
          self.emit('add-cancel', items);
        }
        paused = cancel;
      }
      this.emit('add-before', items, pause);
      if (!paused) {
        if (!_.isNumber(index)) {
...
function doRemove(index) {
  ...
  if (item) {
    ...
    function pause(cancel) {
      if (cancel) {
        self.emit('remove-cancel', item);
      }
      paused = cancel;
    }
    this.emit('remove-before', item, pause);
    if (!paused) {
    ...
    if (items) {
      if (_.isArray(items)) {
        ...
        for (i = 0; i < this.items.length; i++) {
          if (_.isEqual(this.items[i], item)) {
            found = this.items[i];
            break;
          }
        }
        if (found) {
          index = _.indexOf(this.items, found);
          doRemove.call(this, index);
        }
      }
    }
...
function remove(items) {
  return _remove.call(this, items);
}
/**
 ...
 * @param {Function} pause The callback to pause an add operation
 * @param {Boolean} pause.cancel Invoke pause with `cancel = true` to cancel the add operation.
 */
/**
 ...
 * To do this use `pause(true)` in `add-before` handler.
 * @param {Object} item The skipped item.
 */
/**
 * @event remove-before Fired before the remove event.
 * @param {Object} item The removed item.
 * @param {Function} pause The callback to pause a remove operation
 * @param {Boolean} pause.cancel Invoke pause with `cancel = true` to cancel the remove operation.
 */
/**
 ...
 * To do this use `pause(true)` in `remove-before` handler.
 * @param {Object} item The removed item.
 */
/**
